...
function createCloudTexture() {
  ...
  const ctx = canvas.getContext("2d");
  [[40, 38, 22], [65, 30, 26], [90, 36, 20], [55, 42, 18], [75, 44, 16]].forEach(([bx, by, r]) => {
    const grad = ctx.createRadialGradient(bx, by, 0, bx, by, r);
    grad.addColorStop(0, "rgba(255,248,232,0.95)");
    grad.addColorStop(0.7, "rgba(255,244,222,0.7)");
...
function addClouds(scene) {
  ...
  const clouds = [];
  for (let i = 0; i < 6; i++) {
    const mat = new THREE.SpriteMaterial({ map: cloudTex, transparent: true, opacity: 0.8, depthWrite: false });
    const sprite = new THREE.Sprite(mat);
    const scale = 8 + Math.random() * 6;
    sprite.scale.set(scale, scale * 0.5, 1);
    sprite.position.set((Math.random() - 0.5) * 80, 16 + Math.random() * 8, -25 - Math.random() * 25);
    scene.add(sprite);
    clouds.push({ sprite, speed: 0.15 + Math.random() * 0.25, range: 45 });
  }
  return clouds;
}

// ---------- ISLANDS / COASTLINE ----------
// Simple low-poly landmasses placed at the horizon, kept out of the fog-free near range
// so they read as soft, hazy distant scenery rather than playable/interactive terrain.
function buildIsland(x, z, scale, color) {
  const group = new THREE.Group();
  const mat = new THREE.MeshToonMaterial({ color, gradientMap: toonGradient3D });
  const peakCount = 3 + Math.floor(Math.random() * 2);
  for (let i = 0; i < peakCount; i++) {
    const h = (1.4 + Math.random() * 1.6) * scale;
    const r = (1.1 + Math.random() * 0.7) * scale;
    const peak = new THREE.Mesh(new THREE.ConeGeometry(r, h, 5), mat);
    peak.position.set((Math.random() - 0.5) * scale * 2.2, h / 2 - 0.4, (Math.random() - 0.5) * scale * 1.3);
    group.add(peak);
  }
...
function addIslands(scene) {
  [
    buildIsland(-20, -20, 3.2, "#3e5c52"),
    buildIsland(16, -24, 2.4, "#4a6b5e"),
    buildIsland(26, -14, 1.8, "#3e5c52")
  ].forEach(isl => scene.add(isl));
}
...
function createBoard3D(canvas) {
  ...
  const camera = new THREE.PerspectiveCamera(55, 1.6, 0.1, 300);
  camera.position.set(0, 15, 21); // centered on the board, angled but not top-down
  camera.lookAt(0, 0, 0);
...
function animateBoards(time) {
  ...
  board.clouds.forEach(c => {
    c.sprite.position.x += c.speed * deltaSec;
    if (c.sprite.position.x > c.range) c.sprite.position.x = -c.range;
  });
