...
// Small low-poly missile — a thin body, a bright nose cone, and four tail fins, built from
// the same primitive-combining technique used for the ships. Built pointing along local +Y
// by default; launchMissileTo() rotates it to match its actual direction of travel.
function buildMissile() {
  const group = new THREE.Group();
  const bodyMat = new THREE.MeshToonMaterial({ color: 0xd8dde3, gradientMap: toonGradient3D });
  const tipMat = new THREE.MeshToonMaterial({ color: 0xffb84b, gradientMap: toonGradient3D });
  const finMat = new THREE.MeshToonMaterial({ color: 0x4a5560, gradientMap: toonGradient3D });

  const bodyLen = 0.32;
  const body = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, bodyLen, 6), bodyMat);
  group.add(body);

  const noseLen = 0.14;
  const nose = new THREE.Mesh(new THREE.ConeGeometry(0.035, noseLen, 6), tipMat);
  nose.position.y = bodyLen / 2 + noseLen / 2;
  group.add(nose);

  for (let i = 0; i < 4; i++) {
    const fin = new THREE.Mesh(new THREE.BoxGeometry(0.012, 0.08, 0.05), finMat);
    const angle = (i / 4) * Math.PI * 2;
    fin.position.set(Math.cos(angle) * 0.045, -bodyLen / 2 + 0.02, Math.sin(angle) * 0.045);
...
// Removing an object from the scene doesn't free its GPU resources on its own — this
// disposes every mesh's geometry/material so repeated firing over a long game doesn't
// slowly leak memory (worth doing now that a missile is several meshes, not just one).
...
function launchMissileTo(board, target, onImpact) {
  ...
  missile.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), travelDir); // point the nose along the actual flight path
  ...
  board.activeMissiles.push({ missile, trail, start: start.clone(), end, birth: performance.now(), duration: 700, onImpact, done: false });
}
function updateMissiles(board) {
  for (let i = board.activeMissiles.length - 1; i >= 0; i--) {
    ...
    if (t >= 1 && !m.done) {
      m.done = true;
      board.scene.remove(m.missile); disposeObject3D(m.missile);
      board.scene.remove(m.trail); m.trail.geometry.dispose(); m.trail.material.dispose();
      m.onImpact();
...
function animateBoards(time) {
  ...
  const FOAM_THRESHOLD = 0.32; // much closer to the theoretical peak (~0.38) — foam only on genuine rare crest tips
  const FOAM_RANGE = 0.05; // narrow band so foam reads as small distinct sparkles, not a broad gradient
  for (let i = 0; i < board.waterPos.count; i++) {
    ...
    const foam = Math.max(0, Math.min(0.6, (h - FOAM_THRESHOLD) / FOAM_RANGE));
